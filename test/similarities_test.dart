...
void main() {
  ...
  test("similarity is at least -1", () {
    final series1 = List.generate(100, (_) => Event("title", start: randomDate()));
    final series2 = List.generate(30, (_) => Event("title", start: randomDate()));
  ...
  test("similarity of identical series is 1", () {
    final series = List.generate(30, (_) => Event("title", start: randomDate()));
    final result = similarity(series, series);
    expect(result, equals(1.0));
  });

  test("similarity of non-overlapping series is -1", () {
    final series = List.generate(30, (_) => Event("title", start: randomDate())).toSet().toList();
  ...
  test("similarity of series with different length can be calculated", () {
    final series1 = List.generate(randomGenerator.nextInt(50), (_) => Event("title", start: randomDate()));
    final series2 = List.generate(randomGenerator.nextInt(50), (_) => Event("title", start: randomDate()));
    final result = similarity(series1, series2);
    expect(result, isA<double>());
  });

  test("similarity of identical series is greater than similarity of same pair with one unit displaced", () {
    final series1 = List.generate(30, (_) => Event("title", start: randomDate()));
    final series2 = [...series1.sublist(0, 29), Event("title", start: series1.last.start.add(const Duration(days: 1)))];
