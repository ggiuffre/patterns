...
void main() {
  ...
  group("similarity", () {
    test("is at least -1", () {
      ...
      final series2 = randomEvents(20, title: "title", fromYear: 2005, toYear: 2007);
      final result = similarity(series1, series2);
      expect(result, greaterThanOrEqualTo(-1));
    });

    test("is at most 1", () {
      final series1 = randomEvents(50, title: "title", fromYear: 2005, toYear: 2007);
      final series2 = randomEvents(30, title: "title", fromYear: 2005, toYear: 2007);
      final result = similarity(series1, series2);
      expect(result, lessThanOrEqualTo(1.0));
    });

    test("of identical series is 1", () {
      final series = randomEvents(30, title: "title", fromYear: 2005, toYear: 2007);
    ...
    test("of series with different length can be calculated", () {
      final series1 = randomEvents(randomInt(max: 50) + 2, title: "title", fromYear: 2005, toYear: 2007);
      final series2 = randomEvents(randomInt(max: 50) + 2, title: "title", fromYear: 2005, toYear: 2007);
      final result = similarity(series1, series2);
      expect(result, isA<double>());
    });

    test("of identical series is greater than similarity of same pair with one unit displaced", () {
      final series1 = randomEvents(30, title: "title", fromYear: 2005, toYear: 2007);
      final series2 = [
        ...series1.sublist(0, series1.length - 1),
        Event("title", value: randomDouble(), start: series1.last.start.add(const Duration(days: 1)))
      ];
  ...
  group("interpolated", () {
    test("returns a list containing all the events that it was passed", () {
      final events = randomEvents(randomInt(max: 50) + 2, title: "title", fromYear: 2005, toYear: 2007);
      expect(interpolated(events), containsAll(events));
    });

    test("returns events with date in given range, if given only two events", () {
      final event1 = Event("title", value: randomDouble(), start: DateTime.utc(1999, 3, 2));
      final event2 = Event("title", value: randomDouble(), start: DateTime.utc(1999, 6, 7));
    ...
    test("returns events with value in given range, if given only two events", () {
      final event1 = Event("title", value: randomDouble(max: 10), start: DateTime.utc(1999, 3, 2));
      final event2 = Event("title", value: randomDouble() + 10, start: DateTime.utc(1999, 6, 7));
      final result = interpolated([event1, event2]);
    ...
    test("adds events with value 0, if passed binary=true", () {
      final events = randomEvents(randomInt(max: 50) + 2, title: "title", fromYear: 2005, toYear: 2007);
      expect(interpolated(events, binary: true).where((e) => !events.contains(e)).every((e) => e.value == 0), isTrue);
    });
  });
  ...
  group("correlation", () {
    test("is at least -1", () {
      ...
      final series1 = Iterable.generate(nValues, (_) => randomDouble(max: 100)).toSet();
      final series2 = Iterable.generate(nValues, (_) => randomDouble(max: 100)).toSet();
